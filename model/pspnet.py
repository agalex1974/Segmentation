...
import segmentation.model.resnet as models
...
class PSPNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), BatchNorm=nn.BatchNorm2d, pretrained=True):
        ...
        for n, m in self.layer4.named_modules():
            if 'conv2' in n:
                m.dilation, m.padding, m.stride = (2 * grids[countConvolutions], 2 * grids[countConvolutions]), (
                2 * grids[countConvolutions], 2 * grids[countConvolutions]), (1, 1)
