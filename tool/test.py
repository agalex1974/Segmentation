...
def check(args):
    ...
    assert args.split in ['train', 'val', 'test', 'pca']
...
def main():
    ...
    #colors = np.loadtxt('/home/agalex/PycharmProjects/torch_1_3_0/segmentation/dataset/voc2012/voc2012_colors.txt').astype('uint8')

    names = [line.rstrip('\n') for line in open(args.names_path)]
    #names = [line.rstrip('\n') for line in open('/home/agalex/PycharmProjects/torch_1_3_0/segmentation/dataset/voc2012/voc2012_names.txt')]
    if not args.has_prediction:
        ...
        if args.split != 'pca':
          ...
          return pca(test_loader, test_data.data_list, model, args.test_h, args.test_w, mean, std)
    if args.split not in ['test', 'pca']:
        cal_acc(test_data.data_list, gray_folder, args.classes, names, args.scale_image)
...
def net_process_pca(model, image, mean, std=None):
    input = torch.from_numpy(image.transpose((2, 0, 1))).float()
    if std is None:
        for t, m in zip(input, mean):
            t.sub_(m)
        ...
        for t, m, s in zip(input, mean, std):
            t.sub_(m).div_(s)
    input = input.unsqueeze(0).cuda()
    print('input shape pca:', input.shape)
    with torch.no_grad():
...
def pca(pca_loader, data_list, model, network_height, network_width, mean, std):
  model.eval()
  for i, (input, _) in enumerate(pca_loader):
    input = np.squeeze(input.numpy(), axis=0)
    image = np.transpose(input, (1, 2, 0))
    image_scale = cv2.resize(image, (network_width, network_height), interpolation=cv2.INTER_LINEAR)
    print(image_scale.shape)
    return net_process_pca(model, image_scale, mean, std)
...
def cal_acc(data_list, pred_folder, classes, names, scale=1.0):
    ...
    for i, (image_path, target_path) in enumerate(data_list):
        ...
        dim = (width, height)
        target = cv2.resize(target, dim, interpolation=cv2.INTER_NEAREST)
